...
def reveal(board, display_board, row, col, visited):
    if not (0 <= row < len(board)) or not (0 <= col < len(board[0])) or visited[row][col]:
        return
...
def play_game(rows, cols, mines):
    ...
    while True:
        ...
        action = input("Enter action (reveal/flag) and row and column (e.g. 'reveal 1 2'): ").split()
        ...
        if not (0 <= row < rows) or not (0 <= col < cols):
            print(f"Invalid row or column. Please use values between 1 and {rows} for rows and 1 and {cols} for columns.")
            continue
