...
public class NormalMode {
	...
	protected void createContents() {
		...
		btn_back.setBounds(613, 832, 132, 44);
       ...
       //记录剩余雷数和剩余非雷地块数  用于游戏逻辑判断
		...
		mineNum.setFont((Font) localResourceManager.create(FontDescriptor.createFrom("Microsoft YaHei UI", 10, SWT.BOLD)));
		...
		timerLabel.setFont((Font) localResourceManager.create(FontDescriptor.createFrom("Microsoft YaHei UI", 10, SWT.BOLD)));
		...
		if (length * length >= num) {
			...
			tishi.setFont((Font) localResourceManager.create(FontDescriptor.createFrom("Microsoft YaHei UI", 15, SWT.BOLD)));
			tishi.setAlignment(SWT.CENTER);
			tishi.setForeground((Color) localResourceManager.create(ColorDescriptor.createFrom(new RGB(245, 255, 250))));
