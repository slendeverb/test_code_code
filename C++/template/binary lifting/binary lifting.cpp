...
// 读完题后我们马上就能想到一种选法，那就是选 n 个重量为1的胡萝下，这样就能通过加减表示出[1,n]内的所有重量了
// 但问题是......这样挑选的胡萝卜是不是太多了点?
// 我们很快就能发现，只需要选择重量为 1,2,4,8,16 的胡萝下，就能表示[1,31]内的所有重量......
// 只需要选择重量 1,2,4...2^i的胡萝下，就能表示[1,2^(i+1) -1]内的所有重量。
// 也就是说，对于给定的数字n，根本不需要选那么多胡萝下，只需要 [log_2(n)] ([]为向下取整个胡萝卜就够啦!
...
// 这时候我们想起了从引例一得出的结论:
// 只需要记录跳 1,2,4,8,16...2^([log_2(m)]) 次分别能够拿到的胡萝下数，就能得到跳[1,m]区间内任何一个数字能拿到的胡萝卜数
