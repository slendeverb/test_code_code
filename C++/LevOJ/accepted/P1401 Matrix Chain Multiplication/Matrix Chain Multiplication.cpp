...
// 这里，p[i - 1] 是矩阵 Ai 的行数，p[k] 是矩阵 Ak 的列数，p[j] 是矩阵 Aj 的列数。根据矩阵乘法的定义，计算 Ai 到 Ak 的连乘结果矩阵（假设为 M）和 Ak+1 到 Aj 的连乘结果矩阵（假设为 N）时，M 矩阵的大小是 p[i - 1] 行 × p[k] 列，N 矩阵的大小是 p[k] 行 × p[j] 列。为了计算 M 和 N 的乘积，需要进行 p[i - 1] * p[k] * p[j] 次实数乘法。
