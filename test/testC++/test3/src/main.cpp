    ...
    while(true){
        ...
        for(float theta=0;theta<2*pi;theta+=theta_spacin){
            ...
            for(float phi=0;phi<2*pi;phi+=phi_spacin){
                ...
                float lum=(cos_phi*cos_theta*sinB-cosA*cos_theta*sin_phi-sinA*sin_theta+cosB*(cosA*sin_theta-cos_theta*sinA*sin_phi));
                if(lum<0){
                    lum=0;
                }
                ...
                if(position>=0 && position<screen_width*screen_height){
                    if(ooz>z_buffer[position]){
                        ...
                        if(theta>pi+0.2*std::cos(phi*5) && theta<2*pi-0.2*std::cos(phi*5)){
                            c[position]=95;
                        }
                    }
                }
