...
void demo_qt::openActionSlot(){
    QString fileName=QFileDialog::getOpenFileName(this,"选择一个文件",QCoreApplication::applicationDirPath()+"/../src","*.cpp");
    if(fileName.isEmpty()){
...
void demo_qt::saveActionSlot(){
    QString fileName=QFileDialog::getSaveFileName(this,"选择一个文件",QCoreApplication::applicationDirPath()+"/../src");
    if(fileName.isEmpty()){
        qDebug()<<"请选择一个文件";
    }else{
        QFile file(fileName);
